# 成果発表レポート

> 記入者: 山田汰知
> グループ: 1-G
> 日付: 2026/05/27

> **📝 このレポートをそのまま発表原稿にできます。**
> 各セクションの指示を消して、自分の言葉で書いてください。
> 1人 **約2分**（グループ5人で10〜12分）に収まる分量が目安です。

---

## 1. 何を作ったか（30秒）

<!-- ひと言で伝わるように書いてください。「○○を使って、△△するガジェットを作りました。」 -->

**ガジェット名：**
簡単に操作できるモーター

**ひと言で説明：**
スティック操作でモーターの回転方向・速度を変えられるガジェット

**使った部品：**
- アナログジョイスティック
- 直列モーター
- LCDディスプレイ
- 緑LED
- L293D

---

## 2. 設計で考えたこと（15秒）

<!-- 要件定義・基本設計・詳細設計の中で、自分なりに考えたこと・工夫した点を書いてください -->
<!-- 「なぜこの部品を選んだか」「なぜこの構成にしたか」など、判断の理由を言葉にしてください -->
アナログジョイスティックを選んだ理由として、ボタン機能と360度の入力機能の２つ機能がその割っていることで、他の部品との組み合わせが多いと考えたためです。


---

## 3. できたこと・できなかったこと（30秒）

<!-- 正直に書いてください。「できなかった」も立派な成果です -->

**動いたもの：**
- スティック操作でモーターの制御と、LCDディスプレイによる速度モードの表示

**動かなかった・間に合わなかったもの：**
- 
---

## 4. 一番苦労したこと、どう乗り越えたか（30秒）

<!-- ここが発表の山場です。1つだけに絞って、ストーリーで書いてください -->

**何が起きたか：**
L293Dの配線が足りておらず、モーターが動かなかった

**どう対処したか：**
AIや仕様書、TrackLMSを参考に、各ピンの役割とそれらの接続先を要確認した。

**そこから何がわかったか：**
AIに聞くのが一番手っ取り早く簡単だが、考慮する前提要素が多く、ほしい答えが返ってこないことがある。

---

## 5. 学んだこと・今後の展望（25秒）

<!-- 「勉強になった」ではなく具体的に。 -->
<!-- 例: 「AIが生成したコードをそのまま使ったら動かず、自分でprintfデバッグして原因を特定した」 -->
<!-- 例: 「配線図を書かずに組んだら混乱した。図を書いてからやり直したらすぐ動いた」 -->

**学んだこと：**
- ピンの役割を理解しない状態で設計書通りに配線しても、

**今後の展望（この仕組みを発展させるなら）：**
<!-- 例: 「温度センサー＋モーターの組み合わせを応用すれば、室温に応じて自動で換気する仕組みが作れそう」 -->

- ジョイスティック+モーターの組み合わせを応用すれば、難しい操作を必要としない重い荷物を移動できる仕組みが作れそう

---

## 6. 発表で見せたいもの（メモ）

<!-- 動くデモ、配線の写真、設計図、フローチャート、AIとのやり取りのスクショなど -->
<!-- 動くものがなくても、「考えた過程」を見せられれば十分です -->

- 動画

---

> **💡 書き終わったら**
> - 声に出して読んで、2分に収まるか確認してください
> - 長すぎたら「4. 苦労したこと」を1つに絞りましょう
> - グループで導入・締めをつけたい場合は、次の時間に相談して決めてください
