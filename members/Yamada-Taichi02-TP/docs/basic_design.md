...
|:--|:--|:--|:--|:--|:--|
| currentState | 現在の状態（0:待機 1:前転 2:後転 3:停止） | int | 2B | 0 | 状態遷移用 |
| joystickInput | スティックの入力値 | int | 2B | 0 | アナログ値（0〜1023） |
| buttonState | ボタンの状態（ON/OFF） | bool | 1B | false | 電源管理用 |
| motorSpeed | モーターの回転速度 | int | 2B | 0 | 速度制御用 |
| ledState | LEDの状態（RGB値） | int[3] | 6B | {0, 0, 0} | 電源状態表示用 |
...
| F04 | 電源ON/OFF制御 | `togglePower()` | ボタンで電源状態を切替 | なし | なし | loop()内 |
| A01 | 電源状態をRGB LED表示 | `showPowerLED()` | 電源状態に応じてLED色を変更 | なし | なし | loop()内 |
| A02 | 回転速度をLCD表示 | `showSpeedLCD()` | 現在の回転速度をLCDに表示 | なし | なし | loop()内 |
| A03 | 障害物検知 | `detectObstacle()` | 超音波センサーで障害物を検知 | なし | bool | loop()内 |
...
| ボタン入力監視 | 50ms | **millis** | 電源や速度切替の即時反応のため |
