...
| グループ内で統一すべき仕様 | 共通部品のピン番号・スティックのデッドゾーン（実機調整値: LOW=132 / HIGH=892） |
...
| モーター | 3-6V Motor | L293D 5 / 6 | 出力 | 実機配線で確定 |
| LED（緑） | Green LED 5PCS | D13 | 出力 | 実機配線で確定 |
...
|:---|:---|:---|:---|:---|
| 1 | ジョイスティック前倒しでモーター前転 | moveForward() | モーター:L293D 5/6, ジョイスティック:A0 | テスト#1 |
| 2 | ジョイスティック後倒しでモーター後転 | moveBackward() | モーター:L293D 5/6, ジョイスティック:A0 | テスト#2 |
| 3 | 回転速度モード切替 | setSpeedMode() | ボタン:D2, LCD:A4/A5 | テスト#3 |
| 4 | LED表示 | setup(), blinkErrorLED() | LED:D13 | テスト#4 |
...
|:---|:---|:---|:---|:---|:---|
| 1 | ジョイスティック前倒しで前転 | スティックを前に倒す（入力値が132未満） | モーターが前転する |  | [ ] |
| 2 | ジョイスティック後倒しで後転 | スティックを後ろに倒す（入力値が892超） | モーターが後転する |  | [ ] |
